...
`results_df` читается из `ug_exp_results` и содержит строки `client`, `segment`, `variation_pair`, `metric`, `control`, `test`, `diff`, `diff, %`, `ci_low`, `ci_high`, `pvalue`, `color`.

`stats_df` читается из `ug_exp_stats` и содержит строки `client`, `segment`, `metric`, `variation`, `value`.

Обе таблицы сортируются по указанным полям, все значения возвращаются строками. Метрики форматируются через `prefix`/`suffix` из `metrics.yaml`, `diff, %` получает знак `%`, а `pvalue` использует табличное округление. Поле `color` в `results_df` использует ту же p-value раскраску, что Confluence-таблица.
...
13. Воронки добавляются в `ug_experiment_calculator/funnels.yaml` и `ug_experiment_calculator/queries`.
