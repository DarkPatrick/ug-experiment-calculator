...
Пакет считает метрики и воронки A/B-экспериментов Ultimate Guitar, сохраняет результаты в ClickHouse и умеет генерировать JavaScript-конфиги графиков Apache ECharts для рассчитанных метрик.
...
- ECharts реализован только для обычных метрик; графики для воронок нужно добавлять отдельно.
...
6. Публичные импорты: `ug_experiment_calculator/__init__.py`.
7. Метрики добавляются в `ug_experiment_calculator/metrics.yaml`.
8. Воронки добавляются в `ug_experiment_calculator/funnels.yaml` и `ug_experiment_calculator/queries`.
