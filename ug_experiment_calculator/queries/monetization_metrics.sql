...
--         uniqIf((subscription_id, product_id, toDate(subscribed_dt)), subscribed_dt between exp_start_dt and `eut`.exp_start_dt + 604800 and toDate(charge_dt) = toDate(subscribed_dt) and trial = 0 and duration_count > 0) as access_instant_cnt,
--         uniqIf((subscription_id, product_id, toDate(subscribed_dt)), subscribed_dt between exp_start_dt and `eut`.exp_start_dt + 604800 and trial > 0 and toDate(charge_dt) = toDate(subscribed_dt) and duration_count > 0) as access_ex_trial_cnt,
--         uniqIf((subscription_id, product_id, toDate(subscribed_dt)), subscribed_dt between exp_start_dt and `eut`.exp_start_dt + 604800 and trial > 0 and not (toDate(charge_dt) = toDate(subscribed_dt)) and duration_count > 0) as access_trial_cnt,
--         uniqIf((subscription_id, product_id, toDate(subscribed_dt)), subscribed_dt < exp_start_dt and trial > 0 and not (toDate(charge_dt) = toDate(subscribed_dt)) and toDateTime(first_charge_expected_dt) + interval 1 day > exp_start_dt and duration_count > 0) as active_trial_cnt,
...
--         uniqIf((subscription_id, product_id, toDate(subscribed_dt)), subscribed_dt between exp_start_dt and `eut`.exp_start_dt + 604800 and trial > 0 and not (toDate(charge_dt) = toDate(subscribed_dt)) and toDateTime(charge_dt) between toDateTime(subscribed_dt) and toDateTime(first_charge_expected_dt) + interval 1 day and duration_count > 0) as charged_trial_cnt,
--         uniqIf((subscription_id, product_id, toDate(subscribed_dt)), subscribed_dt < exp_start_dt and not (toDate(charge_dt) = toDate(subscribed_dt)) and toDateTime(charge_dt) between toDateTime(subscribed_dt) and toDateTime(first_charge_expected_dt) + interval 1 day and duration_count > 0) as active_charged_trial_cnt,
--         uniqIf((subscription_id, product_id, toDate(subscribed_dt)), subscribed_dt between exp_start_dt and `eut`.exp_start_dt + 604800 and trial = 0 and service_name = '' and duration_count = 0) as access_otp_cnt,
--         uniqIf((subscription_id, product_id, toDate(subscribed_dt)), subscribed_dt between exp_start_dt and `eut`.exp_start_dt + 604800 and trial > 0 and toDate(charge_dt) = toDateTime(0) and toDateTime(cancel_dt) between toDateTime(subscribed_dt) and toDateTime(first_charge_expected_dt) + interval 1 day and duration_count > 0) as cancel_trial_cnt,
...
--         uniqIf((subscription_id, product_id, toDate(subscribed_dt)), subscribed_dt between exp_start_dt and `eut`.exp_start_dt + 604800 and toDateTime(charge_dt) > toDateTime(first_charge_expected_dt) + interval 1 day) as late_charged_cnt,
...
--         uniqIf((subscription_id, product_id, toDate(subscribed_dt)), subscribed_dt between exp_start_dt and `eut`.exp_start_dt + 604800 and toDateTime(charge_dt) between toDateTime(subscribed_dt) and toDateTime(first_charge_expected_dt) + interval 1 day) as charge_cnt,
--         uniqIf((subscription_id, product_id, toDate(subscribed_dt)), subscribed_dt between exp_start_dt and `eut`.exp_start_dt + 604800 and toDateTime(charge_dt) between toDateTime(subscribed_dt) and toDateTime(first_charge_expected_dt) + interval 1 day and toDateTime(refund_dt) between toDateTime(charge_dt) and toDateTime(charge_dt) + interval 14 day) as refund_14d_cnt,
...
--         uniqIf((subscription_id, product_id, toDate(subscribed_dt)), subscribed_dt < exp_start_dt and charge_dt >= subscribed_dt) as recurrent_charge_cnt,
...
--         uniqIf((subscription_id, product_id, toDate(subscribed_dt)), upgrade_dt between exp_start_dt and `eut`.exp_start_dt + 604800) as upgrade_cnt,
...
--         uniqIf((subscription_id, product_id, toDate(subscribed_dt)), subscribed_dt between exp_start_dt and `eut`.exp_start_dt + 604800 and toDateTime(charge_dt) between toDateTime(subscribed_dt) and toDateTime(first_charge_expected_dt) + interval 1 day and toDateTime(cancel_dt) between toDateTime(charge_dt) and toDateTime(charge_dt) + interval 14 day) as cancel_14d_cnt,
